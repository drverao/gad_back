...
//    @Query("SELECT DISTINCT i FROM Indicador i JOIN i.subcriterio s JOIN s.criterio c WHERE c.id_criterio IN :idCriterios")
//    List<Indicador> indicadoresPorCriterios(List<Long> idCriterios);

//    @Query("SELECT DISTINCT i FROM Indicador i " +
//            "JOIN i.subcriterio s " +
//            "JOIN s.criterio c " +
//            "JOIN asignacion_indicador ai ON ai.id_indicador = i.id_indicador " +
//            "JOIN a.modelo m " +
//            "WHERE c.id_criterio IN :idCriterios " +
//            "AND m.id_modelo = (SELECT MAX(mm.id_modelo) FROM Modelo mm)")
//    List<Indicador> indicadoresPorCriteriosUltimoModelo(List<Long> idCriterios);
