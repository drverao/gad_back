...
public interface Ponderacion_repository extends JpaRepository<Ponderacion, Long> {
    ...
    @Query("SELECT p, s.criterio.id, s.criterio.nombre, s.id, s.nombre FROM Ponderacion p JOIN p.indicador i JOIN i.subcriterio s WHERE p.visible = true AND p.modelo.id_modelo = :id_modelo")
    List<Object[]> listarPonderacionConCriterioYSubcriterioPorModelo(@Param("id_modelo") Long id_modelo);
