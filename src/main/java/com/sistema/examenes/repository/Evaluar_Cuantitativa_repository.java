...
public interface Evaluar_Cuantitativa_repository extends JpaRepository<Evaluar_Cuantitativa, Long> {
     ...
     */
    @Query(value = "SELECT ec.* FROM public.indicador i JOIN public.encabezado_evaluar ev ON ev.indicador_id_indicador = i.id_indicador JOIN public.evaluar_cuantitativa ec ON ec.encabezado_evaluar_id_encabezado_evaluar = ev.id_encabezado_evaluar WHERE i.id_indicador= :id_indicador", nativeQuery = true)
